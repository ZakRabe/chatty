...
jQuery.fn.wymeditor = function(options) {
  ...
  options = jQuery.extend({
    ...
    dialogImageHtml: "<div class='wym_dialog wym_dialog_image'>"
		+ "<ul>\n"
		+ "<li><a href=\"#image-upload\"><span>Upload New Image</span></a></li>\n"
		+ "<li><a href=\"#image-server\"><span>Choose Server Image</span></a></li>\n"
		+ "<li><a href=\"#image-url\"><span>Enter Image URL</span></a></li>\n"
		+ "</ul>\n"
		
		+ "<div id=\"image-upload\">"
		+ "<form action=\"/admin/upload/create\" method=\"post\" enctype=\"multipart/form-data\">"
		+ "<div class='row'>"
		+ "<label>Choose image to upload</label>"
		+ "<input id=\"ytUpload_image\" type=\"hidden\" value=\"\" name=\"Upload[image]\" /><input name=\"Upload[image]\" id=\"Upload_image\" type=\"file\" />"
		+ "</div>"
		+ "<div class='row'>"
		+ "<label>Alternative Text</label>"
		+ "<input type='text' class='upload-alt' value='' size='40' />"
		+ "</div>"
		+ "<div class='row'>"
		+ "<label>Image Title</label>"
		+ "<input type='text' class='upload-title' value='' size='40' />"
		+ "</div>"
		+ "<div class='row row-indent'>"
		+ "<input type='button' class='upload-submit' value='Upload' />"
		+ "<input class='wym_cancel' type='button' value='Cancel' />"
		+ "</div>"
		+ "</form>"
		+ "</div>"
		...
		+ "<div id=\"image-server\">"
		+ "<div><input type='text' class='search-text' />"
		+ "<input type='button' class='search-button' value='Search' />"
		+ "<input type='button' class='showall' value='or Show All' />"
		+ "</div>"
		+ "<div id=\"server-image-output\"></div>"
		+ "</div>"
		...
		+ "<input type='text' class='wym_src' value='' size='35' />"
		+ "</div>"
		+ "<div class='row'>"
		+ "<label>Alternative Text</label>"
...
WYMeditor.editor.prototype.container = function(sType) {
  ...
  if(sType) {
    ...
    if(sType.toLowerCase() == WYMeditor.TH) {
      ...
      WYMeditor.H6,WYMeditor.PRE,WYMeditor.BLOCKQUOTE,WYMeditor.DIV);
      container = this.findUp(this.container(), aTypes);
      ...
      if(container) {
        ...
        if(sType.toLowerCase() == WYMeditor.BLOCKQUOTE) {
        ...
        else if(sType.toLowerCase() == WYMeditor.DIV) {
        
          var div = this.findUp(this.container(), WYMeditor.DIV);
          ...
          if(div == null) {
            ...
            if(lgt > 0) firstNode = nodes.item(0);
            for(var x=0; x<lgt; x++) {
              div.parentNode.insertBefore(nodes.item(0),div);
            }
...
WYMeditor.editor.prototype.dialog = function( dialogType, bodyHtml ) {
  ...
  {
    case WYMeditor.DIALOG_IMAGE:
		$('.wym_dialog_image').tabs();
		$('#image-upload input.upload-submit').click(function(){
			$("input.upload-submit").addClass("loading");
			$("input.upload-submit").attr("value", "Uploading...");
			$("input.upload-submit").attr("disabled", "disabled");
			$.ajaxFileUpload({
				...
				success: function (data, status) {
					if (data.error) {
						alert(data.error);
						$("input.upload-submit").removeClass("loading");
						$("input.upload-submit").attr("value", "Upload");
						$("input.upload-submit").attr("disabled", false);
						return false;
					}
					$("input.wym_src").attr("value", data.url);
					$("input.wym_alt").attr("value", $("input.upload-alt").attr("value"));
					$("input.wym_title").attr("value", $("input.upload-title").attr("value"));
					$("#image-url input.wym_submit").trigger("click");
				},
			...
			//$('#image-upload form').trigger('submit'); //debug
			return true;
		});

		function init_imageserver_links() {
			
			$("#server-image-output td a").click(function(){
				
				var imgurl = $(this).attr("href");
				$("input.wym_src").attr("value", imgurl);
				$("input.wym_alt").attr("value", "");
				$("input.wym_title").attr("value", "");
				$("#image-url input.wym_submit").trigger("click");
				return false;
			});
			
			$(
				"#image-server input[type=button],"
				+ "#image-server a.prev,"
				+ "#image-server a.next"
			).click(function(){
				
				$("#server-image-output").html(
					"<img src='"+WYMeditor.BASE_PATH
					+"plugins/imgmgr/ajax-loader.gif' /> Loading..."
				);
				
				if ($(this).attr("class").indexOf("showall") > -1) 
					$("#image-server input.search-text").attr("value", "");
				
				var searchtext = $("#image-server input.search-text").attr("value");
				
				var resultstart;
				if ($(this).attr("href") != "")
					resultstart = $(this).attr("href");
				
				$.getJSON(
					'/admin/upload/find', 
					{ "search": searchtext, "resultstart": resultstart }, 
					function(json){
						
						if (json.error) {
							$("#server-image-output").html("Error: "+json.error);
							return false;
						}
						
						json.start = json.start * 1;
						json.end = json.end * 1;
						json.total = json.total * 1;
						
						var prev = json.start == 1
							? ""
							: "<a href=\""+(json.start-20)+"\" class=\"prev\">&lt; Previous</a> | ";
						var next = json.end == json.total
							? ""
							: " | <a href=\""+(json.start+20)+"\" class=\"next\">Next &gt;</a>";
						var nav = "<p>"
							+ prev
							+ "Showing "+json.start+" to "+json.end+" of "+json.total+" files"
							+ next
							+ "</p>";
						$("#server-image-output").html(
							nav
						...
						for (var x = 0; x < json.images.length; x++) {
							$("#server-image-output tbody").append(
								"<tr>"
								+ "<td class=\"col1\">"+json.images[x].thumbnail+"</td>"
								+ "<td class=\"col2\">"+json.images[x].filename+"</td>"
								+ "<td class=\"col3\">"+json.images[x].filesize+"</td>"
								+ "<td class=\"col4\">"+json.images[x].dimensions+"</td>"
								+ "<td class=\"col5\">"+json.images[x].filetype+"</td>"
								+ "</tr>\n"
							);
						}
						if (json.end != json.total)
							$("#server-image-output").append(nav);
						init_imageserver_links();
					}
				);
